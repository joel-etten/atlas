 ...
 * @Last Modified time: 2018-08-18 16:32:04
 */
...
export const formatEventForCardDisplay = (item) => ({
  ...item,
  startingDay: getDayOfMonth(item.beginning, item.timezone),
  month: getMonth(item.beginning, item.timezone),
  startingDayOfTheWeek: getDayOfWeek(item.beginning, item.timezone),
  categoriesNames: item.categories.map((category) => category.title).toString().replace(',', ', '),
...
const formatEventDateRange = ({beginning, ending, timezone}) => {
  if (isToday(beginning) && isSameDay(beginning, ending)) {
  ...
  if (isTomorrow(beginning) && isSameDay(beginning, ending)) {
